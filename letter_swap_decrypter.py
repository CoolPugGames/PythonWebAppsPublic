...
# Define the filename
WORDLIST_FILENAME = os.path.join(current_dir, 'words_old.txt')
WORD_DICTIONARY_FILENAME = os.path.join(current_dir, 'words_dictionary.json')
...
#     # Now words_list contains all the words from the file
...
class Message(object):
    def __init__(self, text):
        ...
        # self.valid_words_dictionary = load_dictionary(WORD_DICTIONARY_FILENAME)
        # self.valid_words = list(self.valid_words_dictionary.keys())
    ...
    def get_valid_words_dictionary(self):
        return self.valid_words_dictionary.copy()
...
class CiphertextMessage(Message):
    def __init__(self, text):
        ...
        for i in self.split_words:
            no_punct = i.strip(self.punctuation)
    ...
    def decrypt_longest_words(self, lyst, end_index):
        ...
        next_dict_list = []
        for dyct in lyst:
            dyct_score, dyct_one_words = self.get_dict_score_end_compare(dyct.dyct, end_index)
            # replaced test_message.get_dict... with self.get_dict...
    ...
    def find_next_best_dict(self, word, dyct):
        ...
        for dyct in dict_list:
            ...
            print('message so far: ', self.decrypt_final_message(dyct.dyct)) 
            # Replaced test_message.decrypt... with self.decrypt...
...
if __name__ == '__main__':  
    ...
    result = decrypt_message(encrypted_message=encrypted_message)
    print('Your Results, sir: ', result[0])
